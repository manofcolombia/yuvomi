...
- **Recipe, meal, and shopping list quantities can show as fractions.** A household toggle (Settings → Modules → Meals, off by default) renders "1 ½" instead of "1.5" wherever a quantity is displayed - recipe ingredient lists, the ingredient picker used when sending a recipe or meal to a shopping list, and shopping list rows. The stored text and every editable field are unchanged; the toggle only affects how it's rendered.
- **Recipes and meals let you pick which ingredients go to the shopping list.** "Add to shopping list" used to commit the whole ingredient list to one list in a single step. It now opens a checklist - everything is checked by default, so the common case is unchanged - with the target list chosen in the same dialog when the household has more than one list, instead of committing everything or nothing.
- **A planned meal on the Overview page opens its recipe directly.** Tapping a meal in the "Today's meals" widget or the day's program used to always land on the Meal Plan week view. When the meal is linked to a saved recipe, it now opens straight to that recipe's detail; a meal with no linked recipe still opens the Meal Plan, as before.
- **A shopping list item can be moved to another list.** Its edit dialog gains a "List" field (shown once the household has more than one shopping list) - useful for splitting a trip across stores after the fact, not just at add-time. The item keeps its name, quantity, category, notes, and checked state; only its list changes.

### Fixed

- **The household's Week Start setting now applies everywhere a week or date appears, not just the Calendar module.** The setting (Settings → Modules → Calendar → View - Monday/Sunday/Saturday) already changed the Calendar's own month and week grids; it now also anchors the Meal Plan's week view and the date picker used everywhere a date is entered, including when creating or editing a calendar event, which previously always started its mini calendar on Monday regardless of the setting.
